...
@section('title')
    @if (session('role') == 'admin_daskrimti')
        <title>Admin Daskrimti - Valid Permohonan</title>
    @elseif (session('role') == 'admin_umum')
        <title>Admin Umum - Valid Permohonan</title>
    @endif
@endsection
            ...
            <div class="col-12 col-md-6 order-md-2 order-first">
                <nav aria-label="breadcrumb" class="breadcrumb-header float-start float-lg-end">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">Permohonan Valid</li>
                    </ol>
                </nav>
            </div>
        </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            $('#tablePermintaan').DataTable({
                processing: true,
                serverSide: true,
                ajax: '{{ url('getPermintaanByStatusAdmin/1') }}',
                order: [
                    [1, 'desc']
                ],
                columns: [{
