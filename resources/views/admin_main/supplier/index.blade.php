...
@section('title')
    @if (session('role') == 'admin_daskrimti')
        <title>Admin Daskrimti - Supplier</title>
    @elseif (session('role') == 'admin_umum')
        <title>Admin Umum - Supplier</title>
    @endif
@endsection
...
@section('content')
    ...
    <section class="section">
        ...
        {{-- modal update --}}
        @include('admin_main.supplier.update')

        {{-- include modal insert --}}
        @include('admin_main.supplier.insert')
        ...
        $(document).on('click', '.btnDelete', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    window.location.href = '/deleteSupplier/' + id;
        ...
        function view(id) {
            $.ajax({
                ...
                success: function(response) {
                    $('#id_supplier').val(id);
                    $('#nama_supplier').val(response.nama);
                    $('#no_hp_supplier').val(response.no_hp);
                ...
                columns: [{
                    ...
                    },
                    {
                        render: function(data, type, row) {
                            var buttons =
                                '<button style="margin-right: 10px"; class="btn btn-warning btn-sm mr-2" onclick="view(' +
                                row.id + ')"><i class="fa-regular fa-pen-to-square"></i></button>';
                            buttons += '<button data-id="' + row.id +
                                '" class="btn btn-danger btnDelete btn-sm" onclick="hapus(' + row
                                .id + ')"><i class="far fa-trash-alt"></i></button>';
                            return buttons;
